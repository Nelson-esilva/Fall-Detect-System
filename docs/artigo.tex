...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Divisão estratificada:} separação 80/20 (treino/teste) preservando a proporção entre classes.
\end{enumerate}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Conversão TFLite:} exportação do modelo para TensorFlow Lite com quantização INT8 opcional, reduzindo o tamanho e acelerando a inferência em CPU;
...
O modelo foi treinado durante 30 épocas sobre o conjunto de 888 amostras de treino e avaliado em 223 amostras de teste. A Tabela~\ref{tab:metricas} resume as métricas obtidas no conjunto de teste.
...
\textbf{Perda final (loss)} & \multicolumn{4}{c}{$3{,}28 \times 10^{-4}$} \\
\bottomrule
\end{tabular}
\end{table}
...
A matriz de confusão (Tabela~\ref{tab:confusao} e Figura~\ref{fig:confusao}) mostra que o modelo classificou corretamente todas as 223 amostras, sem falsos positivos nem falsos negativos.
...
\midrule
\textbf{Real Normal} & 171 & 0 \\
\textbf{Real Fall}   & 0   & 52 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Visualização da matriz de confusão no conjunto de teste (171 verdadeiros negativos, 52 verdadeiros positivos, 0 falsos positivos, 0 falsos negativos).}
  \label{fig:confusao}
\end{figure}
...
\begin{enumerate}[nosep]
  \item \textbf{Volume limitado:} 70 vídeos originais (30 quedas + 40 ADLs) expandidos para 1\,111 amostras via janela deslizante. Amostras vizinhas compartilham a maioria dos quadros, o que pode inflacionar métricas quando o split treino/teste ocorre por janela (e não por vídeo);
...
\midrule
\citet{kwolek2014} & SVM + profundidade + acelerômetro & 98,33\% & 100\% & 96,67\% \\
\citet{kwolek2014} & Limiar (UFT) & 95,00\% & 100\% & 90,00\% \\
\citet{sisfall2017} & Limiar C8 (SisFall) & 96,10\% & 95,54\% & 96,38\% \\
\textbf{Presente trabalho} & \textbf{CNN-LSTM (MobileNetV2)} & \textbf{100\%} & \textbf{100\%} & \textbf{100\%} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Dataset ampliado:} a inclusão de datasets complementares (SisFall, Kaggle Fall Detection) e a coleta de dados próprios com câmeras reais é necessária;
  ...
  \item \textbf{Hardware embarcado:} a execução do modelo diretamente no ESP32 ou em um Raspberry Pi eliminaria a dependência do PC;
  \item \textbf{Notificações em background:} a Etapa~4 do aplicativo mobile (notificações push quando em segundo plano) ainda está pendente de implementação.
\end{itemize}
...
Como trabalhos futuros, destacam-se a validação do modelo em ambientes domésticos reais com participantes idosos, a ampliação do dataset com dados próprios e de fontes complementares, a migração da inferência para dispositivos de borda (\textit{edge}) e a finalização do módulo de notificações push no aplicativo mobile. O código-fonte completo do projeto está disponível em \url{https://github.com/Nelson-esilva/Fall-Detect-System}.
